 ...
 *
 * Returns binary file content with the correct Content-Type.
 * Used by DocViewer to render images via <img src="...">.
 */
...
export async function GET(request: Request) {
  ...
  try {
    ...
    webLog.api("GET", `/api/files/raw?path=${relativePath}`, 200, Date.now() - start);
    ...
    if (err && typeof err === "object" && "code" in err && err.code === "ENOENT") {
      webLog.api("GET", `/api/files/raw?path=${relativePath}`, 404, Date.now() - start);
      return Response.json(
        { error: "File not found" },
        { status: 404 }
      );
    }
    webLog.api("GET", `/api/files/raw?path=${relativePath}`, 500, Date.now() - start);
    return Response.json(
