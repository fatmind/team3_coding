...
### qoderwork design hero_accessories 

注：模型是旗舰 opus 4.6

``` markdown
1. **草稿是结构契约**：草稿里画了什么块就做什么块，没画的全局容器（侧栏、Topbar、FilterBar、Dialog、Toast 等）默认不要加。跨页跳转优先用最轻量的方式（卡片整块 link、顶部链接），不要默认上 shell。

2. **可以补，但补在内容里，不补外壳**：草稿是示意稿，确实需要补充 —— 但补的应该是页面内的字段、状态、空/错/加载态、视觉细节（token、字号、间距、折叠交互），而不是包在外面的应用外壳和顺手加的功能（手动触发、批次切换、确认弹窗等）。

3. **复杂布局慎用**：能用单列居中容器解决的，不上 grid 分栏 / sticky shell，避免在 Canvas 渲染里跑偏。

4. **交付前自检一句**："把我新增的装饰性外壳删掉，是不是更接近草稿？" 答案是 yes 就删掉再交付。
```