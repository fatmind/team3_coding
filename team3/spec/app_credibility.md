...
3. **换个价格低的模型，这套流程还能不能跑通**
   - 验证项：换 qwen3.7 跑一遍，看看流程是不是只靠 opus 4.6 撑着【已验证】CodeCli 适配层完成，qodercli + qwen3.7 跑通
...
### 问题 6：UAT 黑盒约束未落地，失败后缺少自查[重点][done]
...
### 便宜模型：换 qwen3.7 跑一遍 [重要][done]

已验证。claude code + qwen3.7 跑通整体，相比 claude code + 4.6 opus 质量会下降、人介入更多，但可完成。
...
### 问题 3：Agent 执行中无法打断、插入最新要求[重点][done]
